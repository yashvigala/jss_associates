...
function ConstructionTower({ progress, palette }) {
  ...
  useFrame((state) => {
    ...
    // floors stack, crane-dropped from above
    const N = tower.levels.length
    for (let i = 0; i < N; i++) {
      const obj = levelRefs.current[i]
      if (!obj) continue
      const start = 0.06 + (0.74 * i) / N
    ...
    // crown + beacon land last
    if (crownRef.current) {
      const local = easeOutCubic(clamp01((p - 0.84) / 0.08))
